...
#111 line should not be new_crd!!!
"""
...
#    mini = numpy.argmin(val)
    ...
    h = h5py.File("../mc-ori/lst2.h5",'r')['patt']
    for i in xrange(start,stop):
        patt_exp = h[(i,)]
        mintry = sum(sum(abs(patt_exp-patt)))
        if mintry < minval:
            minval = mintry
            mini = i
    return mini,minval/(scr_size**2)
...
now_crd = (5.0,7.0,7.0)
for n_iter in range(int(numpy.ceil(all_n_iter/check_iter))):
    for i in range(ncpus):
        jobs[i] = job_server.submit(one_core,(check_iter,ltheta,lfai,nowt,nowf,nowdiff,temp,step,s,allarr,sx,sy,sz,now_crd),(find_nbr,),("numpy","h5py"))
    #(nowdiff,nowt,nowf,temp,step,slogdiff,slogdis,n_iter)
    lnowdiff = [x()[0] for x in jobs]
    ind = numpy.argmin(lnowdiff)
    nowt,nowf,temp,step = jobs[ind]()[1:5]
    glogdiff.extend(jobs[ind]()[5])
    glogdis.extend(jobs[ind]()[6])
    all_real_iter += jobs[ind]()[7]
    glog.append(lnowdiff)
    print str(n_iter*check_iter)+'\t'+str(lnowdiff[ind])+'\t' +str(temp)
...
dur = time.time()-time1
print 'dur time: ' + str(dur)  + '\t' +str(all_real_iter)+'\t'+str(dur/all_real_iter)
print temp
print nowt
print nowf
